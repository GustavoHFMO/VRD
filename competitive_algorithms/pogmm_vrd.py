...
class POGMM_VRD(PREQUENTIAL_SUPER):
    ...
    def run(self, labels, stream, cross_validation=True, fold=5, qtd_folds=30, plot=False):
        ...
        ##################################### 3. ######################################################
                ...
                self.CLASSIFIER.virtualAdaptation(x, y, Val, i, plot=False)
                ...
                if(self.POOL_REUSING): 
                    if((True in drifts) and (self.NORMAL_SIGNAL) and self.EVALUATION):
                ...
                if(self.DRIFT_SIGNAL):
                    ...
                    # deactivating the warning procedures
                    self.NORMAL_SIGNAL = False
                    # activating the gaussian evaluation
                    self.EVALUATION = True
                    # reseting the window
                ...
                if(self.POOL_REUSING): 
                    if((True in drifts) and (len(Train) < self.TRAIN_SIZE) and (len(Train) >= self.VAL_SIZE) and self.EVALUATION):
                        #print("Drift")
                        # evaluating the data into the validation window
